...
**最終更新**: 2026-05-25（v4.13 + 引き継ぎ書追加更新）

**現状 HEAD**:
- medadapt: `6ab2a12`（v4.13 docs + HANDOVER 反映済 → さらに今コミットで上書き予定）
- adapt: `a9a93f2`（やるゼ!親LP）
- one-touch: `bfb326a`（ｔやるゼ!LP）
...
### 🚨 新規 chat で「続ける」と言われたら、まず §8-2「次回 chat 即対応タスク」を見ること

大下さんは前回セッション末尾で **4 つの明確な修正指示** を残している:
- ⚡A. ｍやるゼ！の **マッサージ系ページの導線が不明** — 項目が見つからない
- ⚡B. やるゼ！親LPの HERO 文字が **顔と重なる**（写真透けは OK）
- ⚡C. ｔやるゼ！LP の HERO 文字も **顔と重なる**
- ⚡D. ｍやるゼ！HERO も **写真透けスタイル**に揃える

詳細は §8-2 を参照。大下さんが PAT を共有してくれたらすぐ 4 件着手する。
...
|---|---|---|---|
| medadapt | `c7b9cb4`（v4.13） | myaruze.tamjump.com | `docs/DESIGN_yaruze_v4_13_2026-05-25.html` |
| adapt（やるゼ!親） | `a9a93f2` | yaruze.tamjump.com | 同上の §64 で言及 |
| one-touch（ｔやるゼ!） | `bfb326a` | tyaruze.tamjump.com | 同上の §64 で言及 |

3兄弟LP すべて v4.13 でブランド統一完了済み。本番デプロイは GitHub Pages 経由（1-2分で反映）。

### 8-2. ⚡ 次回 chat 即対応タスク（大下さん明示指示・最優先）

> 大下指示（2026-05-25 v4.13 セッション末尾）:
> 「引き継ぎ書を更新して、新規chatで再開する。
> ・特に、ｍやるゼ！のマッサージ系のページの導線が分からない。項目が見つけられない。
> ・親もｔもLPページの写真の顔に文字が重なってる。画像が透き通ってるのはOKだけど。ｍも透き通ってていい。
> どちらにせよ、新規chatで修正するから記載しておいて」

#### ⚡A. ｍやるゼ！ app.html のマッサージ系ページ導線改善（最優先）

**症状**: 大下さん本人が `app.html` でマッサージ系（療養費・同意書・施術報告書）のページに辿り着けない。項目が見つけられない。

**調査ポイント**:
- `app.html` のサイドバーメニュー一覧（現状は L751 付近で定義）:
  ```
  patients / admin / staff / discharge / nda / consent / claims / settings
  ```
- v4.10 で新規作成した acupuncture / massage 同意書テンプレへのリンクが、UI のどこからも辿れていない可能性
- consent（同意書）メニューがあるが、その中で「医師交付同意書（はり・きゅう / マッサージ）」のサブメニュー or 切り替えが存在しない可能性
- 鍼灸師ポータル（`acupuncturist-portal.html`）への導線も要確認

**対応案**:
1. サイドバーに「マッサージ・鍼灸」専用メニューを新設
2. または既存「同意書」メニュー内に「種別: 依頼書 / はり・きゅう / マッサージ」のタブ追加
3. 「加算管理」または別途「療養費」メニューも検討
4. 関連帳票（施術報告書、療養費支給申請書、療養費同意書交付料 B013 算定）への導線も整理

**確認手順（新規chat冒頭）**:
```bash
cd /home/claude/medadapt
# サイドバーメニュー定義の確認
grep -n "patients.*admin.*staff" app.html
# consent ページの実装範囲
grep -n "S.page=='consent'\|'consent.*l:" app.html
# 鍼灸/マッサージへの参照
grep -n "acupuncture\|massage\|療養費\|施術報告" app.html | head -20
# 鍼灸師ポータルがどこから開かれるか
grep -n "acupuncturist-portal" app.html *.html
```

#### ⚡B. やるゼ！親LP（adapt）のHEROタイトル文字と顔の重なり修正

**症状**: スクショ確認 — 「面倒なこと、全部やるゼ！」の「**面倒**」「**全部**」が、画面中央〜右にいる人物の顔（後藤先生・他メンバー）と一部重なっている。
...
**対応案**:
1. `background-position: center 28%` を調整（顔エリアを画面の上か下に追いやる）
2. または `.lp-hero::after` の白オーバーレイをグラデで右端まで濃くして「顔エリアを白く飛ばす」
3. もしくはタイトルを左寄せに強制（max-width で右半分にテキスト来ないように）+ 顔が右半分に集まるよう background-position 調整

**対象ファイル**: `adapt/lp.html` の `.lp-hero::after` グラデ + `.lp-hero::before` の background-position
**現状コード位置**: lp.html L76 付近の HERO CSS（§13-4 参照）
**該当コミット**: adapt HEAD `a9a93f2`

#### ⚡C. ｔやるゼ！(one-touch) LP の文字と顔の重なり修正

**症状**: スクショ確認 — 「24時間の/何でも屋、卒業する。」のタイトルが中央の顔（後藤先生・大下さん）と重なっている。

**ユーザー方針**: 親LPと同じ（写真透けOK、顔と文字の重なりNG）

**対応案**:
1. `background-position: center 25%` を `center 15%`（より上にずらして顔を上端に） or `center 75%`（顔を下端に）に変更
2. `.hero::after` の白オーバーレイ濃度を上げる（現状 78%→100%）
3. テキストの背後に白い半透明ボックスを敷く（最終手段）

**対象ファイル**: `one-touch/index.html` の `.hero::before` / `.hero::after`
**該当コミット**: one-touch HEAD `bfb326a`

#### ⚡D. ｍやるゼ！(medadapt) HEROも「写真透ける」スタイルに揃える

**症状**: 現状の medadapt は「上540px = チーム写真完全表示 / 下白背景テキスト帯」の**完全分離型**。
大下さん指示「ｍも透き通ってていい」 = adapt / one-touch と同じ「**写真背景 + 白オーバーレイで透ける**」スタイルに変更したい。

**対応案**:
1. medadapt の `.hero` を全面写真背景化（v4.10〜v4.11 で慎重に避けていた構造に戻す）
2. ただし v4.11 で苦労した「後藤先生・大下さんの顔と文字の重なり」を再発させないよう、白オーバーレイの濃度設計を入念に
3. adapt の左濃94%→右薄40%対角グラデ方式を移植するのが安全
4. または medadapt の HERO テキスト位置を **左半分固定**、写真の顔エリアが右半分に来るよう `background-position` 調整

**対象ファイル**: `medadapt/index.html` L106 付近の `.hero` CSS
**該当コミット**: medadapt HEAD `c7b9cb4`
**注意**: v4.9 → v4.11 で何度か HERO レイアウト試行錯誤があったため、この変更を「v4.14 HEROスタイル統一」として丁寧に扱う
