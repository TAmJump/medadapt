...
**最終更新**: 2026-06-22（v5.4.0 / 保険情報を既定マスク＋権限者のみ全表示＋全表示ログ＋資格確認鮮度警告に作り替え）

**現状 HEAD**:
- medadapt: v5.4.0（保険情報 安全表示 / 本セッション push が HEAD）
...
| **v5.4.0** | 保険情報の安全表示に作り替え（マイナ保険証時代の資格確認HUB化）。①記号番号・介護被保険者番号・公費受給者番号を既定でマスク表示（例 8839＊＊＊／2509＊＊＊＊24）②「全表示」は閲覧権限者（自法人admin/staff＋active共有先）のみ、押下は患者監査ログ『保険番号 全表示』に記録③最終資格確認から30日超で⚠警告・未確認は注意表示④用途・全表示ログ・マイナンバー非保存を明記。CSVに保険番号は元々含めない方針を維持。※フィールド単位のハード制御（非所有法人へ番号を返さない）はWorker側redactが必要＝JINのデプロイ領域 | `本セッション` ★HEAD |
| **v5.3.0** | 導線改善：患者ハブ概要の「この患者で操作」を司令塔化。これまでサイドバーからしか作れず患者を選び直す必要があった 診療情報提供書／居宅療養管理指導／歯科 を、患者ハブから患者プリセット済みで直接モーダル起動できるよう統一（showReferralModal/showKyotakuModal/showDentalModal に presetPid 引数追加）。アクションを「医療文書／介護・在宅／管理」の3グループにラベル分けして一覧性を改善。サイドバー各帳票ページは『一覧・横断閲覧』用途として残置 | `本セッション` ★HEAD |
| **v5.2.3** | デモ/一括用の保険情報ジェネレータ genDemoInsurance を追加。新規デモアカウントの seed 患者全員に医療保険・介護保険・公費（年齢で後期高齢者医療/国保/被用者保険を出し分け、負担割合・記号番号枝番・被保険者番号を決定的に生成）を自動付与。既存アカウントはコンソール一括スニペットで backfill | `本セッション` ★HEAD |
