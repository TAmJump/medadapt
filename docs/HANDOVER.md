...
| 単位の誤用 | 「タブレット1枚」 | 「タブレット／スマホ 1台」（**v4.25 で更新**: 単にタブレット1台でもなく、スマホも含めるのが正） |
...
| 専門点数コードの羅列 | HEROのハッシュタグに「＃療養費同意書交付料(B013)」「＃退院時共同指導加算(B004)」など | （v4.25 で削除）一般ユーザに意味不明なので、点数コードは機能解説セクションの説明文内のみで使用（HEROタグや見出しには出さない） |
